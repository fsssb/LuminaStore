...
- 同机（Apple Silicon arm64）单线程；Qdrant 以 v1.16.1 本地二进制（aarch64）运行，
  延迟为 **qdrant-client 本地回环往返**（含网络+序列化）——这本身就是「服务型 vs 嵌入式」的形态差异
...
| **Qdrant**(服务型) | 50 | 0.9975 | 1640 | 4938 | 517 | 29.1 | - |
| **Qdrant**(服务型) | 100 | 0.998 | 1691 | 1976 | 589 | 29.1 | - |
| **Qdrant**(服务型) | 200 | 0.999 | 2024 | 2648 | 490 | 29.1 | - |
| **Qdrant**(服务型) | 400 | 1.0 | 2511 | 3027 | 403 | 29.1 | - |

### 按 recall 对齐的延迟

| recall@10 | LuminaStore | usearch | Chroma | Qdrant |
|-----------|-------------|---------|--------|--------|
| ~0.99 | **325 us** (ef=100) | 342 us (ef=50) | - | - |
| ~0.9945 | 442 us (ef=200) | **343 us** (ef=100) | - | - |
| ~0.9975+ | 766 us (ef=400) | - | 988 us | **1640 us** (本地往返) |

### 结论（诚实版）

1. **嵌入式 vs 服务型的形态差异被数据证实**：Qdrant 即使本地部署，单查询也要
   **1.6-2.5ms**（网络 + 序列化往返），嵌入式引擎（LuminaStore/usearch）**0.3-0.8ms**——
   **服务型数据库进不了低延迟本地场景**，这正是嵌入式形态存在的意义。
2. **LuminaStore 与 SIMD 库 usearch 处于同一性能带**：低 recall 区延迟更优，高 recall 区
   p50 落后 ~30%，但 p99 更好（565 vs 612 us @ recall≈0.9945）；usearch 构建快 1.6x。
3. **LuminaStore 显著优于同形态嵌入式数据库 Chroma**：同 recall 下延迟 2-3x 更低。
4. **usearch 是库不是数据库**：无持久化/过滤/快照/多语言生态——LuminaStore 的能力组合是差异化。
5. **JSON 绑定开销**：LuminaStore 的 Python 延迟含 ctypes+JSON 序列化；C++ 直连路径无此开销。
...
# Qdrant（本地二进制，v1.16.1 aarch64）：先启动服务再测
./bench/embedded/qdrant &                       # 监听 localhost:6333
.venv/bin/python bench/embedded/bench_qdrant.py data/mnist/base.fvecs data/mnist/query.fvecs --nq 200 --ef "50,100,200,400"
# 汇总
